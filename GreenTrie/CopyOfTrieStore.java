...
public class CopyOfTrieStore implements ExpressionStore {
	...
	public CopyOfTrieStore(Green solver, Properties conf) {
		super();
		if (config != null) {
			CopyOfTrieStore.config = config;
		}
		String basePath = (String) CopyOfTrieStore.config
				.get("constraint.store.basePath");
		if (basePath != null) {
			CopyOfTrieStore.basePath = basePath;
		}
		satisfiableTrie = (Trie) FileUtil.readObject(CopyOfTrieStore.basePath,
				"satisfiableTrie");
		unsatisfiableTrie = (Trie) FileUtil.readObject(CopyOfTrieStore.basePath,
				"unsatisfiableTrie");
	...
	public void shutdown() {
		FileUtil.saveObject(CopyOfTrieStore.basePath, "satisfiableTrie",
				satisfiableTrie);
		FileUtil.saveObject(CopyOfTrieStore.basePath, "unsatisfiableTrie",
				unsatisfiableTrie);
	}
	...
	public void report(Reporter reporter) {
		...
		reporter.report(getClass().getSimpleName(), "SAT_ConstraintCount = "
				+ satisfiableTrie.getPatternCount());
		reporter.report(getClass().getSimpleName(), "UNSAT_ConstraintCount = "
				+ unsatisfiableTrie.getPatternCount());
...
//				SubSetTask subSetTask2 = new SubSetTask(satisfiableTrie,
//						expList);
