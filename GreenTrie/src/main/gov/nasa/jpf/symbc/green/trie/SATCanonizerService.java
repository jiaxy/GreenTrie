...
//					o = (Operation) e;
					if (o.getOperator() == Operation.Operator.GT) {
						e = new Operation(Operation.Operator.GE, 
								merge(o.getOperand(0), new IntConstant(-1)),
								o.getOperand(1));
...
// added  by jia xiangyang	
		...
		private boolean isFirstExpNegtive(Operation ex) {
			...
			if(ex!=null&&ex.getOperator()== Operation.Operator.MUL){
				Expression op0 =ex.getOperand(0);
				if(op0 instanceof IntConstant){
					if(((IntConstant) op0).getValue()<0){
...
// removed by Jia Xiangyang, keep the form: (x+0<=0)		
//			else if (s == 0) {
//				return lr;
//			}
			else {
