...
public class RouteTracker {
	...
	/** Sole Constructor */
	public RouteTracker(ServerEngine engine, Calculator calculator, LatLonBounds bounds) {
		this.server = engine;
...
//	 *** Setters and getters ***
	...
	public RouteTrack findRouteTrack(FlightTrack ft, FlightPlan fp) {
		...
		PointXY flightXY = calculator.toXY(flightPoint, bounds);
		PointXY prevXY = calculator.toXY(minPrevFix, bounds);
		PointXY nextXY = calculator.toXY(minNextFix, bounds);
		...
		return new RouteTrack(minPrevFix, minNextFix, minSnapPoint
				...
				calculator.angleLL(minPrevFix, minNextFix, bounds));
	}
	...
	private Point2D snapPointToRouteSegment(Point2D latLonPoint, Point2D fix1,
		...
		PointXY p = calculator.toXY(latLonPoint, bounds);
		PointXY l1 = calculator.toXY(fix1, bounds);
		PointXY l2 = calculator.toXY(fix2, bounds);
		...
		if (((l1.getX() < x && x < l2.getX()) || (l2.getX() < x && x < l1
				...
				&& ((l1.getY() < y && y < l2.getY()) || (l2.getY() < y && y < l1
						.getY()))) {
			return calculator.toLL(intersectXY, bounds);
		}
		...
		return calculator.toLL(minXY, bounds);
	}
}