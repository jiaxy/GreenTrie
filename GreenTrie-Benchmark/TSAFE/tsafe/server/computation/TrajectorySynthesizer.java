...
public class TrajectorySynthesizer  {
	...
	//private ComputationMediator mediator;
	...
	/** Sole constructor */
	public TrajectorySynthesizer(ServerEngine engine,
			Calculator calculator, LatLonBounds bounds) {
		this.server = engine;
...
//	 *** Setters and getters ***
	...
	private Point4D deadReckon(FlightTrack ft, long time) {
		...
		PointXY startXY = calculator.toXY(ft.getLatitude(), ft.getLongitude(),bounds);
	...
	public Trajectory getRouteTrajectory(RouteTrack rt, Route r) {
		...
		double dist = calculator.distanceLL(rt.getLatitude(),
				rt.getLongitude(), nextFix, bounds);
		...
		if (timeToNextFix < getParameters().tsTimeHorizon) {
		...
		if (fixIter.hasNext()) {
			double heading = calculator.angleLL(currPoint.getLatitude(),
					currPoint.getLongitude(), nextFix, bounds);
			...
			Point4D end = deadReckon(lastTrack,
					getParameters().tsTimeHorizon - timeElapsed);
